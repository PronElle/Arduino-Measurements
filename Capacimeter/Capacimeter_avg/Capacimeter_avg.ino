...
void loop() {
  static float C, C_avg = 0;
  static uint16_t i = 0; // 0 --> 1000
  static uint16_t V0, V1;
   ...
   // computing capacity
   C = T  / (Rs * log((float)V0/(float)V1)) * 1000000000.0;
   C_avg += C;
   
   // dislaying C
   Serial.println("measure " + String(i) + " : " + String(C) + " nF");
  
   
   if( i == SAMPLES){
    Serial.println(C_avg/(float)SAMPLES, 4);
